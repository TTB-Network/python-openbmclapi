...
class DashboardData:
    ...
    def deserialize(data: utils.DataInputStream):
        match (data.readVarInt()):
            ...
            case 4:
                return [
                    DashboardData.deserialize(data) for _ in range(data.readVarInt())
                ]
            case 5:
                return {
                    DashboardData.deserialize(data): DashboardData.deserialize(data)
                    for _ in range(data.readVarInt())
                }
            case 6:
                return None
    ...
    def serialize(data: Any):
        ...
        if isinstance(data, str):
            ...
            buf.write(
                b"".join((DashboardData.serialize(v).io.getvalue() for v in data))
            )
            ...
            buf.write(
                b"".join(
                    (
                        DashboardData.serialize(k).io.getvalue()
                        + DashboardData.serialize(v).io.getvalue()
    ...
    async def process(type: str, data: Any):
        ...
        if type == "system":
            return {
                ...
                "cpu": system.get_cpus(),
                "cache": (
                    dataclasses.asdict(await cluster.get_cache_stats())
                    if cluster
                    else StatsCache()
                ),
            }
...
async def init():
    ...
    async def _(ws: web.WebSocket):
        async for raw_data in ws:
            ...
            output.write(
                DashboardData.serialize(
                    await DashboardData.process(type, data)
                ).io.getvalue()
            )
            await ws.send(output.io.getvalue())
