                        ...
                        "refresh": () => {
                            axios.get("dashboard").then(resp => {
                                ...
                                for (hourly of data.hourly) {
                                    ...
                                    file_bytes[hour] = (hourly.bytes / Math.max(hourly.hits, 1) / 1024.0 / 1024.0).toFixed(2)
                                ...
                                core_modules_locals["dashboard"]["file_bytes"] .setOption({title: {text: "每小时平均文件流量（MiB）"}, tooltip:{formatter: e => e[0].data == null ? '' : '<div style="margin: 0px 0 0;line-height:1;"><div style="margin: 0px 0 0;line-height:1;"><div style="margin: 0px 0 0;line-height:1;"><div style="margin: 0px 0 0;line-height:1;"><div style="margin: 0px 0 0;line-height:1;"><span style="display:inline-block;margin-right:4px;border-radius:10px;width:10px;height:10px;background-color:#0fc6c2;"></span><span style="font-size:14px;color:#666;font-weight:400;margin-left:2px">平均文件流量：      </span><span style="float:right;margin-left:20px;font-size:14px;color:#666;font-weight:900">'+e[0].data+' MiB</span><div style="clear:both"></div></div><div style="clear:both"></div></div><div style="clear:both"></div></div><div style="clear:both"></div></div><div style="clear:both"></div></div>'},series: [{data: file_bytes}]})
                                ...
                                document.getElementById("t-d-file_bytes").innerText = ((days.bytes / Math.max(days.hit, 1)) / 1024.0 / 1024.0).toFixed(2) + " M"
                            })
                        },
