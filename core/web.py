...
class Application:
    ...
    async def handle(self, request: "Request"):
        ...
        yield Response(
            content=result or "",
            headers=Header({"Server": "TTB-Network"}),  # type: ignore
        )
...
class Request:
    def __init__(self, data: bytes, client: Client):
        ...
        self.client = client
        self.method, self.url, self.protocol = data.decode("utf-8").strip().split(" ")
    ...
    async def get_method(self):
        if (
            ...
            and self.method == "GET"
            and (await self.get_headers("Connection") or "Closed") == "WebSocket"
        ):
    ...
    while True:
        try:
            server = await asyncio.start_server(
                _handle, host="0.0.0.0", port=port, ssl=cert
            )
            logger.info(
                f"Server listening on {str(port)}{' with ssl' if cert else ''}!"
            )
