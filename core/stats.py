...
from core.utils import FileDataInputStream, FileDataOutputStream, get_timestamp_from_day_tohour, get_timestamp_from_hour_tohour
...
db.execute("create table if not exists `Stats`(Time numeric not null, hits numeric default 0, bytes numeric default 0, qps numeric default 0, sync_hits numeric default 0, sync_bytes numeric default 0)")
db.commit()
...
def read():
    ...
    with open("./cache/stats_count.bin", "rb") as r:
        ...
        while 1:
            try:
                t, h, b, q, sh, sb = f.readVarInt(), f.readVarInt(), f.readVarInt(), f.readVarInt(), f.readVarInt(), f.readVarInt()
                if h == b == q == sh == sb == 0:
                    continue
                caches[t] = Counters(
                    h, b, q, sb, sh
...
def write_database():
    ...
    for t, v in caches.items():
        ...
        cmds.append(("update `Stats` set `hits` = ?, `bytes` = ?, `qps` = ?, `sync_hits` = ?, `sync_bytes` = ? where `Time` = ?", (v.hit, v.bytes, v.qps, v.sync_hit, v.sync_bytes, t)))
...
def hourly():
    ...
    data = []
    for r in queryAllData("select `Time`, `hits`, `bytes`, `qps`, `sync_hits`, `sync_bytes` from `Stats` where `Time` >= ?", t):
        hour = r[0] - t
        data.append(
            {"_hour": int(hour),
             "hits": r[1],
