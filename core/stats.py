...
def read_storage():
    ...
    if not Path("./cache/storage.bin").exists() or Path("./cache/storage.bin").stat().st_size == 0:
        return
...
def _write_database():
    ...
    for storage in storages.values():
        if hour not in last_storages or hour != last_storages[storage.get_name()]:
            if not exists("select storage from access_logs where storage = ? and hour = ?", storage.get_name(), hour):
                cmds.append(
                    ("insert into access_logs(storage, hour) values (?, ?)", (storage.get_name(), hour))
                )
            last_storages[storage.get_name()] = hour
        cmds.append(("update access_logs set hit = ?, bytes = ?, cache_hit = ?, cache_bytes = ?, last_hit = ?, last_bytes = ?, failed = ? where storage = ? and hour = ?", (
            storage._hits,
            storage._bytes,
            storage._cache_hits,
            storage._cache_bytes,
...
def hourly():
    ...
    for r in queryAllData("select storage, hour, hit, bytes, cache_hit, cache_bytes, last_hit, last_bytes, failed from access_logs where hour >= ?", t):
        data.append(
            {
                "_hour": int(r[1] - t),
                "cache_hits": r[4],
