(() => {
    ...
    const popstate = () => {
        ...
        if (last_key != key || (last_key_b && last_key_b != last_key_sub)) {
            if (last_key != null) handler(last_key, last_key_sub, "disconnect")
    ...
    (() => {
        ...
        class Storage {
            constructor() {
                this._page = [
                    ttb.createElement("div").class("panel").append(
                        ttb.createElement("p").class("title").setText(
                            "文件数量"
                        ),
                        ttb.createElement("p").class("value").setText(
                            "-"
                    ...
                    ttb.createTable().header(
                        ...
                        ttb.createElement("p").class("value").setText("共 0 个存储")
                    ).style("margin: 16px")
                ]
            }
            setData(data) {
                console.log(data)
                this._page[1].setData(...data)
            }
            connect(page) {
                ws.send("storage")
                page.push(...this._page)
