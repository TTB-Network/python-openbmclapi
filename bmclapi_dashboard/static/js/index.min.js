...
class Application {
    constructor() {
        ...
        this.$theme = this.createElement("i").class("bx", `bxs-${theme}`).style("font-size: 32px").event("click", () => {
            ...
            document.cookie = `theme=${this.$theme.containsClass("bxs-sun") ? "1" : "0"}`
        }),
...
class ElementFlex extends Element {
    ...
    update(main = true) {
        ...
        const width_avg = Math.max(0, Math.floor((widths - percentWidth) / (this._child - this._customWidths.length)))
        for (let i = Math.min(this._customWidths.length, children.length); i < children.length; i++) {
            const child = children[i];
            child.valueOf().style = this._childStyle
            child.setStyle("boxSizing", "border-box")
...
app.$Menu.add("dashboard", new class {
    ...
    async init() {
        this.page = [
            ...
            app.createFlex().minWidth(896).childWidths(60, 40).append(
                app.createElement("div").append(
                    app.createElement("div").class("panel").append(
