    ...
    constructor() {
        this.themes = {
            light: {
                ...
                "text-shadow": "rgba(15, 198, 194, 0.04)",
                "none-color-shadow": "rgb(10, 138, 135)",
                "close-background": "rgba(0, 0, 0, 0.04)",
                "hover-color": "#0A8A87",
            ...
            dark: {
                ...
                "text-shadow": "rgba(235, 187, 151, 0.04)",
                "none-color-shadow": "rgb(170, 146, 125)",
        ...
        this.setStyles({
            "h1, h2, h3, h4, h5, h6, p": "font-family: system-ui",
...
class Application {
    constructor() {
        ...
        this.$styles = {
            ...
            ".main-container": [
                "transition: width 250ms linear 0s, padding 250ms linear 0s, margin-left 250ms linear 0s",
            ],
        ...
        theme = theme == null ? !window.matchMedia("(prefers-color-scheme: light)").matches : theme
        if (!theme) {
            theme = "moon"
...
class Configuration {
    ...
    save() {
        window.localStorage.setItem("configuration", JSON.stringify(this.data))
    }
    load() {
        this.data = JSON.parse(window.localStorage.getItem("configuration") || "{}")
    }
}
class ProgressBar {
    constructor() {
...
class SocketData {
    ...
    send_http(namespace, data) {
        return new Promise(async (resolve, reject) => {
            ...
            if (resp.ok) {
                ...
                var resp_raw_data = (await resp.json())
                resp_data = [resp_raw_data.err, resp_raw_data.data]
            } else {
                window.dispatchEvent(new Event("mainsocket_disconnect"))
                resp_data = [new TextDecoder("utf-8").decode(resp.body.getReader().read()), null]
            }
            this.recv(namespace, resp_data)
            resolve(resp_data)
        })
    }
    async create_ws() {
        ...
        if (this.$ws == null && !this.$creating) this.$ws = new WebSocketClient("ws" + (this.$url.slice(4)), {
            "onmessage": (msg) => {
                ...
                if (!(data.id in this.$callback)) return
                this.$callback[data.id].resolve([data.err, data.data])
    ...
    send_ws(namespace, data) {
        ...
        return new Promise((resolve, reject) => {
            var id = this.$key++
            if (!this.$opened) this.$cache[namespace] = {id, data}
            this.$ws.sendJSON({
                id: id,
                namespace,
                data
            })
            this.$callback[id] = {resolve, reject}
        })
    }
    ...
    recv(namespace, data) {
        window.dispatchEvent(new CustomEvent("mainsocket_" + namespace, { detail: data }))
    ...
    _serializeData(data) {
        ...
        buf.writeString(JSON.stringify(data))
        return buf
    }
    support() {
        return this.$support
...
class ElementFlex extends Element {
    ...
    update(main = true) {
        ...
        const main_padding = (() => {
            const child_style = window.getComputedStyle(this.valueOf());
            const padding = (parseInt(child_style.paddingRight, 10) + parseInt(child_style.paddingLeft, 10));
            return (Number.isNaN(padding) ? 0 : padding)
        })()
        const width = (super.valueOf().offsetWidth - 1) - main_padding;
    ...
    updateData() {
        ...
        for (const val of data) {
            ...
            const progress = app.createElement("div").class("app-rank-progressbar").append(
                app.createElement("div").class("app-rank-progressbar-value").style(`width: ${total == 0 || val.value == 0 ? 0 : Math.max(1, (val.value / total) * 100.0)}%`)
            )
...
class PanelModal extends Modal {
    constructor() {
        ...
        this._Styles = {
            ...
            ".modal-container": [
                ...
                transition: box-shadow 300ms cubic-bezier(0.4, 0, 0.2, 1) 0ms;
                border-radius: 4px;
                box-shadow: rgba(0, 0, 0, 0.2) 0px 11px 15px -7px, rgba(0, 0, 0, 0.12) 0px 24px 38px 3px, rgba(0, 0, 0, 0.12) 0px 9px 46px 8px;
                margin: 32px;
                position: relative;
                overflow-y: auto;
                display: flex;
            ...
            ".modal-panel-confirm-button:hover": [
                `text-decoration: none;
                background-color: var(--none-color-shadow);`
            ],
            ".modal-panel-cancel-button": [
                `display: inline-flex;
                -webkit-box-align: center;
                align-items: center;
                -webkit-box-pack: center;
            ...
            ".modal-panel-padding-button": [
                `padding-left: 30px;
                padding-right: 30px;`
            ],
            ".modal-panel-cancel-button:hover": [
                `background-color: var(--text-shadow);`
            ],
            ".modal-panel-body": [
                `flex: 1 1 auto;
        ...
        this._header = app.createElement("div").class("modal-panel-header")
        this._title = app.createElement("div").class("modal-panel-title")
        this._close = app.createElement("i").class("bx", "bx-x", "modal-panel-close").event("click", () => {
            this.close()
        })
        this._footer = app.createElement("div").class("modal-panel-footer")
        this._body = app.createElement("div").class("modal-panel-body")
        this._cancel = app.createElement("button").class("modal-panel-cancel-button", "modal-panel-padding-button").setI18N("button.cancel").event("click", () => {
            this.close()
        })
        this._confirm = app.createElement("button").class("modal-panel-confirm-button", "modal-panel-padding-button").setI18N("button.confirm").event("click", () => {
            this._confirm_handler && this._confirm_handler()
            this.close()
        })
        this._confirm_handler = null;
        this._confirm_handler = null
        this.container = [
            this._header.append(
                this._title,
        ...
        app.setStyles(this._Styles)
    }
    body(...elements) {
        this._body.append(...elements)
        return this
    }
    open() {
        this._cancel.setStyle("display", "none")
        this._confirm.setStyle("display", "")
        super.open()
        super.body(...this.container)
        return this
    }
    title(text) {
        this._title.setText(text)
        this._title.setI18N(null)
        return this
    }
    title_i18n(i18n, params) {
        this._title.setI18N(i18n, params)
        return this
    }
    title_t18n(params) {
        this._title.t18n(params)
        return this
    }
    close() {
        this._title.clear()
    ...
    closeHandler(func) {
        this.close_handler = func
        return this
    }
    setCancel(visible) {
        this._cancel.setStyle("display", visible ? "" : "none")
        return this
    }
    setConfirm(visible) {
        this._confirm.setStyle("display", visible ? "" : "none")
        return this
    }
    setConfirmHandler(handler) {
        this._confirm_handler = handler
    }
}
const AlertType = {
    NORMAL: 0,
...
class Alert {
    constructor() {
        this._base = app.createElement("div").class("alert-container")
        this._alerts = []
        this._type = {}
        for (const key in AlertType) {
            this._type[AlertType[key]] = key
        }
    }
    add(type, msg) {
        this._alerts.push(
...
class Authentication {
    constructor() {
        ...
        this._container = app.createElement("div").class("panel-auth").append(
            this.createLabelInput("dashboard.auth.username", this._input.username),
            this.createLabelInput("dashboard.auth.password", this._input.password),
            this._captcha,
            this._input.label,
            this._input.auth,
        ...
        this._Styles = {
            ...
            '.panel-auth .label-input.active label': [
                `max-width: calc(133% - 32px);
                transform: translate(14px, -9px) scale(0.75);
                user-select: none;`
            ],
            '.panel-auth .label-input fieldset': [
                `text-align: left;
                position: absolute;
                inset: -5px 0px 0px;
                margin: 0px;
            ...
            '.panel-auth .label-input legend span': [
                `font-size: 0.75em;
                visibility: hidden;
                white-space: nowrap;`
            ],
            '.panel-auth .label-input.active legend span': [
                `font-size: 0.75em;
                visibility: visible;
                white-space: nowrap;`
            ],
        ...
        this.token = $config.get("Token") || null
        app.setStyles(this._Styles)
    }
    async load() {
    ...
    createLabelInput(label, input) {
        const focus = (event) => {
            main.class("active")
        }
        const focusout = (event) => {
            if (event.target.value != null && typeof event.target.value === 'string' && event.target.value.trim().length != 0) return
            main.removeClass("active")
            event.target.value = ''
        }
        const main = app.createElement("div").class("label-input").append(
            app.createElement("label").setI18N(label),
            app.createElement("fieldset").append(
                app.createElement("legend").append(
                    app.createElement("span").setI18N(label)
                )
            ),
            input.event("focus", focus).event("focusout", focusout)
        )
        this._main.push(main)
        return main
    }
    ...
    close() {
        for (const input of [
            ...
            this._input.captcha.input,
        ]) {
            input.setValue("")
        }
        for (const main of this._main) {
            main.removeClass("active")
        }
        this._input.label.setText("")
    }
...
$I18N.addLangs("zh_cn", {
    ...
    "dashboard.useragents": "客户端下载数",
    "dashboard.status_code": "状态码响应",
    ...
    "dashboard.connections": "连接数",
    "dashboard.basic.summary": "统计总结",
    "dashboard.storage._all_": "所有存储",
    "dashboard.auth": "登陆",
    "dashboard.auth.login": "登陆",
    "dashboard.auth.username": "用户名",
    "dashboard.auth.password": "密码",
    "unit.hourly": "%hour% 时",
    "button.confirm": "确定",
    "button.cancel": "取消",
    "see.more": "查看更多",
    ...
    "menu.config": "配置",
    "menu.config.basic": "基础设置",
    "menu.config.storage": "存储设置",
    "menu.utils": "更多工具",
    "menu.utils.classics": "精华图片",
    "utils.refresh": "刷新",
    "tqdm": "%value%/%total% [%start% < %end%, %item%/s%postfix%]",
...
const fillDates = (dates) => {
    const defaultDate = formatDate((new Date()).valueOf())
    if (!(defaultDate in dates))
        dates[defaultDate] = 0
    const latestDateStr = Object.keys(dates).reduce((latest, current) =>
        new Date(current + "T00:00:00Z") > new Date(latest + "T00:00:00Z") ? current : latest)
...
const fillDateTimes = (dates, day) => {
    const defaultDate = formatDateTime(parseInt(((new Date()).valueOf() / 3600000) + "") * 3600000)
    if (!(defaultDate in dates))
        dates[defaultDate] = 0
    const latestDateStr = Object.keys(dates).reduce((latest, current) =>
        new Date(current + ":00") > new Date(latest + ":00") ? current : latest)
...
const fillTimes = (dates, secs) => {
    const defaultDate = formatTime(parseInt(((new Date()).valueOf() / 1000) + "") * 1000)
    const date = formatTime((new Date()))
    if (!(defaultDate in dates))
        dates[defaultDate] = 0
    const latestDateStr = Object.keys(dates).reduce((latest, current) =>
        new Date(`${date} ${current}`) > new Date(`${date} ${latest}`) ? current : latest)
    const latestDate = (new Date(latestDateStr)).valueOf();
    const filledDates = {};

    for (let i = Object.keys(filledDates).length; i < secs; i++) {
        filledDates[formatTime(new Date(latestDate + (i - secs) * 1000))] = 0
    }
    var result = { ...filledDates, ...dates };
    for (const key in result) {
        result[formatSTime(new Date(key))] = result[key]
    }
    return result;
}
...
app.$Menu.add("dashboard", new class {
    ...
    async init() {
        ...
        this.storages = []
        this.switch_storage = (new ElementSwitch("dashboard.storage._all_")).event("click", (index) => {
            $MainSocket.send("statistics", (this.storages.length != 0 ? this.storages[index - 1] : null)).then((data) => {
                var [err, ack] = data
                this.statistics = ack
                this.storages = ack.storages
                this.drawStatistics()
            })
        })
        this.switch_storage.select(0)
        ...
        this.summary = app.createElement("button").class("click-text").setI18N("dashboard.basic.summary").event("click", () => {
            modal.open().title("统计")
            $progress.set(50)
        ...
        this.summary_container = app.createElement("div").append(
            this.summary
        ).class("flex-jend", "flex", "flex-aligns-center").style("margin-right: 12px")
        ...
        this._r_status = app.createElement("div").class("border-background")
        this.geo_type = "world"
        ...
        this.page = [
            app.createElement("div").class("panel").append(
                ...
                ).minWidth(896).child(2).addResize(() => this.pbar_resize()),
                this.pbar
            ),
        ...
        this.basic = [
            ...
            app.createElement("div").class("panel nopadding").style("margin-bottom: 0; margin-top: 0").append(
                this.switch_storage,
            ),
        ...
        this.pro = [
            ...
            app.createFlex(true).minWidth(1280).childWidths(60, 40).append(
                ...
                app.createElement("div").class("panel").append(
                    app.createElement("p").class("title").setI18N("dashboard.status_code"),
                    app.createFlex(true).minWidth(324).childWidths(30, 70).append(
                        this._e_status,
                        this._r_status
                    )
                ),
        ...
        await $MainSocket.send("statistics", (this.storages[this.switch_storage.selected - 1])).then((data) => {
            ...
            this.statistics = ack
            this.storages = ack.storages
            this.switch_storage.setButtons("dashboard.storage._all_", ...ack.storages)
            this.switch_storage.select(0)
            this.drawStatistics()
        })
    ...
    initBasic() {
        ...
        this.statisticsTimer = app.runTaskRepeat(() => {
            $MainSocket.send("statistics").then((data) => {
                this.statistics = data[1]
                this.drawStatistics()
                this.drawSystem()
            })
        }, 0, 10000)
    ...
    initPro() {
        ...
        var empty_data = fillTimes({}, 60)
        for (let val of [
            ["cpu", this._e_cpu, ],
            ["memory", this._e_memory],
            ["connections", this._e_connection]
        ]) {
            val[1].setOption({
                color: app.getThemeVar("main-color"),
                yAxis: [
                ...
                series: [
                    {
                        name: $I18N.t("dashboard." + val[0]),
                        type: "line",
                        symbol: "none",
                        data: Object.values(empty_data),
                        smooth: true,
                        lineStyle: {
                            width: 3,
                            color: {
    ...
    clearPro() {
        ...
        this.pro[0].getChildren()[0].getChildren()[1].setText("0")
        this.pro[0].getChildren()[1].getChildren()[1].setText("0")
        this.pro[0].getChildren()[2].getChildren()[1].setText("0")
        this.pro[3].getChildren()[0].getChildren()[0].getChildren()[1].t18n({ peer: "0.00%" })
        this.pro[3].getChildren()[1].getChildren()[0].getChildren()[1].t18n({ peer: this._format_bytes(0)})
        this.pro[3].getChildren()[2].getChildren()[0].getChildren()[1].t18n({ peer: 0})
    }
    clearBasic() {
        this.systemTimer?.block()
        this.statisticsTimer?.block()
        this.basic[0].getChildren()[0].getChildren()[0].getChildren()[0].getChildren()[0].getChildren()[1].setText("0")
        this.basic[0].getChildren()[0].getChildren()[0].getChildren()[0].getChildren()[1].getChildren()[1].setText("0.00B")
        this.basic[0].getChildren()[0].getChildren()[0].getChildren()[0].getChildren()[2].getChildren()[1].setText("0")
        this.basic[0].getChildren()[0].getChildren()[0].getChildren()[0].getChildren()[3].getChildren()[1].setText("0.00B")
        this.basic[0].getChildren()[0].getChildren()[1].getChildren()[0].getChildren()[0].getChildren()[1].setText("0")
        this.basic[0].getChildren()[0].getChildren()[1].getChildren()[0].getChildren()[1].getChildren()[1].setText("0.00B")
        this.basic[0].getChildren()[0].getChildren()[1].getChildren()[0].getChildren()[2].getChildren()[1].getChildren()[0].setText("0")
        this.basic[0].getChildren()[0].getChildren()[1].getChildren()[0].getChildren()[2].getChildren()[1].getChildren()[1].setText("(0.00B)")
        this.basic[0].getChildren()[0].getChildren()[1].getChildren()[0].getChildren()[3].getChildren()[1].setText("0.00%")
        this.basic[2].getChildren()[0].getChildren()[0].getChildren()[1].t18n({peer: 0})
        this.basic[2].getChildren()[1].getChildren()[0].getChildren()[1].t18n({peer: 0})
        this.basic[2].getChildren()[2].getChildren()[0].getChildren()[1].t18n({peer: 0})
        this.basic[2].getChildren()[3].getChildren()[0].getChildren()[1].t18n({peer: 0})
        for (var e of [this._e_hourly, this._e_daily]) {
            for (var o of Object.values(e)) {
                o.clear()
            }
    ...
    _format_bytes(size, i = null) {
        ...
        size = size / (1024 ** i)
        return `${r ? '-' : ''}${size.toFixed(2)}${this._unit_bytes[i]}`
    }
    _format_number_unit(n) {
        var d = (n + "").split("."), i = d[0], f = d.length >= 2 ? "." + d.slice(1).join(".") : ""
        return i.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ", ") + f;
    ...
    drawStatistics() {
        ...
        const daily_data = data.daily;
        {
            ...
            let io, cache;
            {
                ...
                this.basic[2].getChildren()[0].getChildren()[0].getChildren()[1].t18n({peer: Math.max(...io.map((ioVal, index) => ioVal + cache[index]), 0)})
                ...
                this.basic[2].getChildren()[1].getChildren()[0].getChildren()[1].t18n({peer: this._format_bytes(Math.max(...io.map((ioVal, index) => ioVal + cache[index]), 0))})
            }
        }
        {
            let io, cache;
            {
                ...
                this.basic[2].getChildren()[2].getChildren()[0].getChildren()[1].t18n({peer: Math.max(...Object.keys(io).map(key => io[key] + cache[key]))})
                ...
                this.basic[2].getChildren()[3].getChildren()[0].getChildren()[1].t18n({peer: this._format_bytes(Math.max(...Object.keys(io).map(key => io[key] + cache[key])))})
            }
        }
    }
    drawQPS() {
    ...
    drawGEO() {
        ...
        if (this.geo_type == "world") {
            ...
            data = this.geo_stats.addresses.filter(e => e.country == "CN").map(e => {
                return {
                    country: e.country,
                    province: e.province,
                    name: e.province,
    ...
    drawSystem() {
        var hits, bytes;
        hits = this.statistics.hourly.reduce((sum, i) => sum + i.hits + i.cache_hits, 0)
        bytes = this.statistics.hourly.reduce((sum, i) => sum + i.bytes + i.cache_bytes, 0)
        ...
        hits = this.statistics.daily.reduce((sum, i) => sum + i.hits + i.cache_hits, 0)
        bytes = this.statistics.daily.reduce((sum, i) => sum + i.bytes + i.cache_bytes, 0)
        ...
        this.basic[0].getChildren()[0].getChildren()[1].getChildren()[0].getChildren()[2].getChildren()[1].getChildren()[1].setText(`(${this._format_bytes(this.system.cache.data_bytes)})`)
    ...
    drawDetailSystem() {
        ...
        this._e_connection.setOption({
            color: app.getThemeVar("main-color"),
            yAxis: {
                max: Math.max(...Object.values(this.detail_system.connections).map(v => Math.ceil(v / 100)), 1) * 100
            },
            xAxis: {
    ...
    drawGlobals() {
        ...
        var data = []
        for (const key in this.pro_stats.status) {
            data.push({
                label: this._status_name_to_code[key],
                value: this.pro_stats.status[key]
            })
        }
        data = data.sort((a, b) => b.value - a.value)
        this._e_status.setOption({
    ...
    setRank_Status() {
        var data = [];
        for (const key in this.pro_stats != null && this.pro_stats.status != null ? this.pro_stats.status : {}) {
            data.push({
                label: this._status_name_to_code[key],
                value: this.pro_stats.status[key]
            })
        ...
        this._r_status.clear()
        var fd = data.slice(0, 5)
        for (var index in fd) {
            let value = fd[index]
            this._r_status.append(
                app.createElement("div").class("flex", "flex-space-between", "flex-aligns-center").style("margin-top: 8px; margin-bottom: 8px").append(
                    app.createElement("div").class("flex", "flex-aligns-center").append(
                        app.createElement("div").class("rank-circle").style(`background-color: var(--echarts-color-${index}); `),
                        app.createElement("span").class("title-color").setText(value.label)
    ...
    setStatus() {
        ...
        if (this.pbar.valueOf().style.display == 'none' && this.status.progress != null) setTimeout(() => this.pbar_resize(), 50)
        ...
        if (this.status.progress) {
            ...
            this.pbar.getChildren()[2].setI18N("tqdm", {
                ...
                postfix: this.status.progress.postfix ? ", " + this.status.progress.postfix : ""
            })
        }
    }
...
app.$Menu.add("config.basic", new class {
    constructor() {
    ...
    async init() {
        this.page = [
            app.createElement("div").class("panel").append(
                app.createElement("p").class("titlebar").setI18N("system.info"),
                app.createElement("div").style("margin-top: 16px").append(
                    app.createElement("span").class("title").setText("cn"),
                    app.createElement("span").class("value").setText("cn")
                )
            ),
        ]
    }
...
app.$Menu.add("utils.classics", new class {
    constructor() {
        this.BASE_URL = (__CONFIG__.bangbang93hub ? "/mirrors/bangbang93hub" : "https://apis.bmclapi.online/api/93") + "/random"
    ...
    requestImage() {
        return new Promise(async (resolve, reject) => {
            ...
            xhr.open("GET", `${this.BASE_URL}?type=json`, true)
            xhr.send()
        })
    }
    async init() {
        this.page = [
            app.createElement("div").class("panel").append(
                app.createElement("div").class("flex", "flex-space-between").append(
                    app.createElement("p").class("titlebar").setText("Fetching..."),
                    app.createElement("div").class("flex", "flex-jend").append(
                        app.createElement("button").class("button").setI18N("utils.refresh").event("click", async () => {
                            await this.refresh()
                        })
                    )
                ),
                app.createElement("div").class("classic_image").append(
    ...
    async refresh() {
        ...
        await this.requestImage().then((data) => {
            this.page[0].getChildren()[0].getChildren()[0].setText(data.name)
            this.page[0].getChildren()[1].getChildren()[0].setAttribute("src", data.data)
        })
        this.refreshing = false
    }
