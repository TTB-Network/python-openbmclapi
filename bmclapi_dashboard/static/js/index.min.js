    ...
    send_http(namespace, data) {
        return new Promise(async (resolve, reject) => {
            ...
            if (resp.ok) {
                ...
                resp_data = [null, (await resp.json()).data]
            } else {
    ...
    create_ws() {
        ...
        if (this.$ws == null) this.$ws = new WebSocketClient("ws" + (this.$url.slice(4)), {
            "onmessage": (msg) => {
                ...
                this.$callback[data.id].resolve([null, data.data])
...
class Authentication {
    ...
    async load() {
        var [err, ack] = await $MainSocket.send("auth", null)
        if (!ack || !ack.success) {
            this.logged = false
        } else {
...
app.$Menu.add("dashboard", new class {
    ...
    async init() {
        ...
        await $MainSocket.send("status", (new Date().valueOf())).then((data) => {
            var [err, ack] = data
            var ping = (new Date().valueOf()) - ack.timestamp
            console.log(ack)
