...
class WebDavStorage(Storage):
    def __init__(self, webdav_url: str, webdav_username: str, webdav_password: str) -> None:
    ...
    async def check_missing_files(self, BMCLAPIfile: Dict[str, List[str]]) -> List[str]:
        ...
        async with Client(**self._options) as client:
            tasks = [self.check_file_existence(client, file_path) for file_path in BMCLAPIfile['files']]
    ...
    async def upload_missing_files(self, missing_files: List[str]) -> None:
        ...
        async with client as webdav_client:
            upload_tasks = [self.upload_file(webdav_client, file) for file in missing_files]
    ...
    async def main(self) -> None:
        BMCLAPIfile = [...]  
        await self.write_files_json({'files': BMCLAPIfile})  # 将文件列表写入到 files.json
