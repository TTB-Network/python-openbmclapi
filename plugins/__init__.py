...
plugins: list[Plugin] = []
def load_plugins():
    with os.scandir("./plugins") as files:
        for file in files:
            if file.name.startswith("__"):
                continue
            try:
                plugin = Plugin(importlib.import_module("plugins." + file.name.rsplit(".", 1)[0]))
                logger.info(f"Plugin Loaded [{plugin.get_name()}]! Version: [{plugin.get_version()}], Author: [{plugin.get_author()}]")
